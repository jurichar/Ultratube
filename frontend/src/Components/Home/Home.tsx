...
export default function Home() {
  ...
  async function get_trending_movie() {
    try {
      const all_movies_res = await fetch(`https://yts.mx/api/v2/list_movies.json?sort=like_count&page=1`, { method: "GET" });
      ...
      if ("movies" in all_movies_json.data) {
        const arrayTrending = all_movies_json.data.movies.map((elem) => {
          return { id: elem.id, title: elem.title, release: elem.year, image: elem.medium_cover_image, synopsis: elem.synopsis, trailer: elem.yt_trailer_code };
        });
  ...
  return (
    <div className="w-full h-max text-quinary p-4 flex flex-col items-center justify-around gap-6">
      ...
      <div className="w-full h-full flex flex-col gap-4 ">
        ...
        <div ref={body} className="flex flex-row flex-wrap gap-5">
          {movies.map((movie) => (
